...
public class TelegramOTPAuthenticator implements Authenticator {
    ...
    private static final String TOTP_SECRET_ATTR = "telegram_totp_secret";
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        try {
            ...
            logger.error("Failed to send Telegram OTP", e);
            Response challenge = context.form()
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        if (context.getHttpRequest().getDecodedFormParameters().containsKey("resend")) {
            ...
            user.setSingleAttribute(TOTP_SECRET_ATTR, newSecret);
            
            String otp = generateOTP(newSecret);
            ...
            try {
                ...
                Response challenge = context.form()
                    .setSuccess("otpResent")
                    .createForm("telegram-otp.ftl");
                context.challenge(challenge);
            } catch (IOException e) {
                logger.error("Failed to resend Telegram OTP", e);
                Response challenge = context.form()
        ...
        // Стандартная обработка OTP
        String enteredOtp = context.getHttpRequest().getDecodedFormParameters().getFirst("otp");
        if (!validateOTP(context.getUser().getFirstAttribute(TOTP_SECRET_ATTR), enteredOtp)) {
            Response challenge = context.form()
                .setError("invalidOtp")  // Используем сообщение из messages_en.properties
                .createForm("telegram-otp.ftl");
